...
export const getReservations = (userId: number, status?: string) =>
  request<any[]>(
    `/reservations?user_id=${userId}${status ? `&status=${status}` : ""}`,
  );

// Admin reservations
export const getPendingReservations = () =>
  request<any[]>("/admin/reservations/pending");
...
// Exams
export const getStudentExams = (studentId: number) =>
  request<any[]>(`/exams/student/${studentId}`);
...
/* export const getStudentDocuments = (studentId: number) => {
  let data: any = request<any[]>(`/documents?student_id=${studentId}`);
  for (let index = 0; index < data.length; index++) {
    if (data[index].student_id === studentId) {
      return data[index];
    }
  }
}; */
...
export const getDocuments = async (status: StatusDocument) => {
  const documents = await request<any[]>("/documents");
  if (status !== "all") return documents.filter((doc) => doc.status === status);
  return documents;
};
